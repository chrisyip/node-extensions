...
var filter = require('./var/arrayFilter')
...
if (isUndefined(proto.insert)) {
  proto.insert = function insert (element, position) {
    var startPos = Math.floor(position)
    this.splice(Number.isNaN(startPos) ? this.length : startPos, 0, element)
    return this
  }
}
...
if (isUndefined(proto.remove)) {
  proto.remove = function remove (element, startPosition) {
    var startPos = Math.floor(startPosition),
        index = this.indexOf(element, Number.isNaN(startPos) ? 0 : startPos)
...
if (!isNativeCode(proto.compact)) {
  proto.compact = function compact (callback, thisArg) {
    return filter(this, isFunction(callback) ? callback : function (item) {
      return item != null
    }, thisArg)
  }
}
...
proto.slice = function (starPos, endPos) {
  return slice(this, starPos, endPos)
}
...
proto.forEachRight = function forEachRight (callback, thisArg) {
  var iterator = thisArg == undefined ? callback : function (elem, i, arr) { return callback.call(thisArg, elem, i, arr) }
...
proto.reduceRight = function reduceRight (callback, initialValue) {
  var i = this.length
...
proto.indexOf = function indexOf (subject, fromIndex) {
  var i = fromIndex >> 0,
      length = this.length
...
proto.lastIndexOf = function lastIndexOf (subject, fromIndex) {
